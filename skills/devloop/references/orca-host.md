...
- 用户 `--host=` / `DEVLOOP_HOST=` → `detect-runtime-host.sh --force <host>`，这是严格选择，不可用即失败；仅偏好且允许降级时用 `--prefer <host>`。
...
## 一个 devloop 一个 Orca worktree（隔离执行区）
...
- 若 F1 goal 已就绪，可用 `worktree create --agent <impl> --prompt <goal>` 一次性创建执行区并点火，但必须仍保持“一个 devloop 一个 worktree”、后续 reviewer/F2 复用同一 devloop worktree 的语义；goal 未就绪时先 bare create，再 `terminal create`。
- 若 CLI 较旧不支持某些 flag：先 `worktree create --name ... --json`，再用 `terminal create`。
- 从 create 响应抄写完整 `worktree.id`（格式 `<repoId>::<path>`），以及 path；写入 `.devloop/<id>/progress.md` 与 plan 头注释。
- 创建后从 `source_worktree` 用 `cp -a` 或 `rsync` 迁入 `.devloop/<id>/`，核对 plan 记录的 `plan_sha256`；执行区必须从锁定的 `base_sha` 起步。
...
- 分支名尽量 `feat/devloop-<id>-<slug>`（Orca/checkout 若另有命名，以实际 branch 为准并记入 plan）。
- `.devloop/` 的 commit/归档与 dirty allowlist 以 plan 的 control-plane 策略为准；impl 避免无边界 `git add -A`。**绝不 push**。

可选状态：

```text
ORCA worktree set --worktree id:<repoId>::<path> --workspace-status in-progress --json
ORCA worktree set --worktree id:<repoId>::<path> --comment "devloop <id> loop: open F1" --json
```

### 已在目标 worktree 内

若协调者 cwd 已是目标 devloop worktree（`ORCA worktree current --json` 路径匹配），可复用，不再 create。

## 小周期 2：扇出 impl agent

goal 已写入 worktree 内绝对路径 `.../.devloop/<id>/next-goal.md` 后：

### 推荐：在 devloop worktree 内新建 agent 终端

```text
ORCA terminal create \
  --worktree id:<repoId>::<devloopWorktreePath> \
  --title devloop-<id>-<slice>-impl-<attempt> \
...
ORCA terminal send --terminal <impl_handle> --text "Read and execute the self-contained goal at <ABS>/.devloop/<id>/next-goal.md. Work only in this worktree. Write the delivery report to <ABS>/.devloop/<id>/run/<impl>-<slice>-report.md. Local commit only; never push. Do not advance plan.md roadmap status." --enter --json
```

F1 在 goal 已就绪时可用 `worktree create --agent <impl> --prompt <ABS-goal>` 点火；该 create 得到的 worktree 就是本 devloop 唯一执行区，后续 reviewer/F2 必须复用它，不得再建“新切片专用子 worktree”。goal 尚未就绪或复用既有 devloop worktree 时，使用 `terminal create`。
...
- 可把关键摘录追加到 `.devloop/<id>/run/<impl>-<slice>.log`（人工/哨兵维护的摘要日志），便于与 tmux 路径产物对齐。

## 小周期 3：扇出 reviewer agent（不同工具）

impl 完成后，主会话写好 acceptance-prompt 绝对路径，再开 **另一条** terminal（**同一** devloop worktree）：

```text
ORCA terminal create \
  --worktree id:<repoId>::<devloopWorktreePath> \
  --title devloop-<id>-<slice>-review-<attempt> \
...
ORCA terminal send --terminal <review_handle> --text "You are the adversarial reviewer (impl=<impl>, reviewer=<reviewer>). Read <ABS>/.devloop/<id>/run/<impl>-<slice>-acceptance-prompt.md and execute it. Write ONLY the acceptance report to <ABS>/.devloop/<id>/run/<impl>-<slice>-acceptance.md with header impl=/reviewer=. Conclusion must be exactly one of: 全部完成 / 未完成 / BLOCKED. Do not change product code, do not commit, do not edit plan.md status." --enter --json
```
...
ORCA worktree set --worktree id:<repoId>::<path> --comment "devloop <id> F2 acceptance running (reviewer=codex)" --json
```
...
<skill>/scripts/devloop cleanup-sessions \
  --orca-cli <orca_cli> \
  --worktree id:<repoId>::<devloopWorktreePath> \
...
- 脚本会关：`worktree ps` 里本 worktree `state=done` 的 agent、title 含 `devloop-<id>-` 的遗留 tab、以及 `--also-close`。
...
- 若用户明确要求用 `orca orchestration` 做 DAG/多 worker 邮箱协调，另开 orchestration skill；默认 devloop loop 只用 `worktree` + `terminal` 子集，降低复杂度。
...
ORCA worktree create --name devloop-<id>-<slug> --parent-worktree active --setup <policy> --json
ORCA worktree set --worktree id:<repoId>::<path> --comment "..." --json
ORCA worktree set --worktree id:<repoId>::<path> --workspace-status in-progress --json
ORCA terminal create --worktree id:<repoId>::<path> --title devloop-<id>-f1-impl-<attempt> --command "claude --permission-mode default" --json
...
<skill>/scripts/devloop cleanup-sessions --orca-cli ORCA --worktree id:<repoId>::<path> --keep <live> --title-prefix devloop-<id>-
```
