...
1. 检查确认记录存在且当前只有一个 `open` 片；否则回到 grill 或暂停。确认记录须含 impl 与 reviewer（或「按默认配对」）。
...
4. 小周期 3：impl runner 退出后，主会话写 acceptance-prompt，启动 **与 impl 不同的 reviewer** 做对抗式阶段验收，写 `.adr/<id>/run/<impl>-<slice>-acceptance.md`。
